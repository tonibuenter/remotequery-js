...
async function resolveIncludes(se, context) {
  ...
  statementList = tokenize(statements.trim(), STATEMENT_DELIMITER, STATEMENT_ESCAPE);
  resolvedList = [];
...
async function parametersCommand(request, currentResult, statementNode, serviceEntry, context) {
  ...
  let result = await processCommandBlock(statementNode, request, currentResult, serviceEntry, context);
...
function isFilteredOut(ignoredErrors, errorString) {
  let isFiltered = (ignoredErrors || []).reduce((a, e) => errorString.includes(e) || a, false);
  return isFiltered;
}
